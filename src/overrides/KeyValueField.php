...
class KeyValueField extends Original
{
    public $separator = ' ';
    ...
    public function KeyField()
    {
        if ($this->sourceKeys && count($this->sourceKeys)) {
            return Object::create($this->keyFieldClassForLists, '[key][]', '',
                $this->sourceKeys)->addExtraClass('mventryfield mvdropdown');
        } else {
            return Object::create($this->keyFieldClass,
                $this->name . '[key][]')->addExtraClass('mventryfield mvtextfield');
        }
    }
    ...
    public function ValField()
    {
        if ($this->sourceValues && count($this->sourceValues)) {
            return Object::create($this->valFieldClassForLists, $this->name . '[val][]', '',
                $this->sourceValues)->addExtraClass('mventryfield mvdropdown');
        } else {
            return Object::create($this->valFieldClass,
                $this->name . '[val][]')->addExtraClass('mventryfield mvtextfield');
        }
    }

    public function Type()
    {
        return strpos(parent::Type(), strtolower(__NAMESPACE__ . '\\')) === 0 ? substr(parent::Type(),
            strlen(__NAMESPACE__ . '\\')) : parent::Type();
    }
